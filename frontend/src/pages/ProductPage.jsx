...
function ProductPage() {
  const { id } = useParams();   // get product ID from URL params (exp: /product/123)
  const { userId } = useAuth();
  const navigate = useNavigate();   // able to navigate

  const { data: product, isLoading, error } = useProduct(id);   // run useProduct hook in useProducts.js
  ...
  if (error || !product) {    // Handle case where product is not found
    return (
        ...
        {isOwner && (
          <div className="flex gap-2">
            <Link to={`/edit/${product.id}`} className="btn btn-ghost btn-sm gap-1">
              <EditIcon className="size-4" /> Edit    {/* Edit button for product owner */}
            </Link>
            <button
              onClick={handleDelete}    // Delete button for product owner
